...
| `/juanstack-init ["name"]`                                    | **Initialize JuanStack vertical.** Conducts an interactive domain interview and configures `business.json`, `PROJECT_CONTEXT.md`, AI manifest, and BIR compliance.                                                         |
...
| `/sync-upstream [path                                         | --commits [N]]`                                                                                                                                                                                                            | Pull DannFlow updates into a `feat/sync-*` PR. `.claude/commands/` is mirrored exactly; required template migrations are detected automatically, applied to the project database, verified, and followed by regenerated types. File-level diff is the default; `--commits` enables cherry-pick mode. |
| `/sync-to-upstream [path\|--dry-run]`                         | Contribute generic improvements back to DannFlow. It automatically validates detected reusable database changes against the dedicated template database before committing, pushing, and opening a PR.                      |
...
| `/migrate <description>`                                      | Creates SQL in `supabase/migrations/`, applies with `npm run db:migrate`, verifies Supabase, and syncs types.                                                                                                              |
| `/schema-change <description>`                                | Explicit live Supabase MCP schema workflow: checkpoint → approved SQL → tracked `supabase/migrations/*.sql` file → apply_migration → sync types → verify.                                                                  |
